...
// "this record advanced past this stage").
//
// Identical to src/purchase/shared/stageChain.js — copied rather than
// cross-imported so the order and purchase systems stay fully decoupled.
//
...
// a downstream (next) stage row already exists, revert is refused so the
// flow can't be broken — the caller must revert the later stage first.
async function revertStage(tx, { model, where, childModel, childWhere, parentModel, parentWhere }) {
  if (childModel) {
    ...
    if (child) {
      const err = new Error('This record has already moved past this stage — revert the later stage first.');
      err.statusCode = 400;
      throw err;
    }
  }
