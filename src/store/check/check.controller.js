...
// @desc    Get all check
// @route   GET /api/store/check
const getAll = async (req, res, next) => {
  try {
    const data = await prisma.storeCheck.findMany({
...
// @desc    Get single check by ID
// @route   GET /api/store/check/:id
const getOne = async (req, res, next) => {
  try {
    const data = await prisma.storeCheck.findUnique({
...
// @desc    Create check
// @route   POST /api/store/check
const create = async (req, res, next) => {
  try {
    const data = await prisma.storeCheck.create({
...
// @desc    Upsert check for its parent record (create if missing, update if it already exists)
// @route   PUT /api/store/check/by-parent/:parentId
const upsertByParent = async (req, res, next) => {
  try {
    ...
    delete data.liftId;
    const record = await prisma.storeCheck.upsert({
...
// @desc    Update check
// @route   PATCH /api/store/check/:id
const update = async (req, res, next) => {
  try {
    const data = await prisma.storeCheck.update({
      where: { id: parseInt(req.params.id, 10) || req.params.id },
      data: sanitizeData(req.body)
    });
    res.json({ success: true, data });
...
// @desc    Delete check
// @route   DELETE /api/store/check/:id
const remove = async (req, res, next) => {
  try {
    await prisma.storeCheck.delete({
      where: { id: parseInt(req.params.id, 10) || req.params.id }
    });
    res.json({ success: true, data: {} });
  } catch (error) {
