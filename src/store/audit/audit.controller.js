...
// @desc    Get all audit
// @route   GET /api/store/audit
const getAll = async (req, res, next) => {
  try {
    const data = await prisma.storeAudit.findMany({
...
// @route   GET /api/store/audit/:id
const getOne = async (req, res, next) => {
  try {
    const data = await prisma.storeAudit.findUnique({
...
// @desc    Create audit
// @route   POST /api/store/audit
const create = async (req, res, next) => {
  try {
    const data = await prisma.storeAudit.create({
...
// @desc    Upsert audit for its parent record (create if missing, update if it already exists)
// @route   PUT /api/store/audit/by-parent/:parentId
const upsertByParent = async (req, res, next) => {
  try {
    ...
    delete data.liftId;
    const record = await prisma.storeAudit.upsert({
...
// @desc    Update audit
// @route   PATCH /api/store/audit/:id
const update = async (req, res, next) => {
  try {
    const data = await prisma.storeAudit.update({
      where: { id: parseInt(req.params.id, 10) || req.params.id },
      data: sanitizeData(req.body)
    });
    res.json({ success: true, data });
...
// @desc    Delete audit
// @route   DELETE /api/store/audit/:id
const remove = async (req, res, next) => {
  try {
    await prisma.storeAudit.delete({
      where: { id: parseInt(req.params.id, 10) || req.params.id }
    });
    res.json({ success: true, data: {} });
  } catch (error) {
