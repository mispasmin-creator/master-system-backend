...
// @desc    Get all management-approval
// @route   GET /api/store/management-approval
const getAll = async (req, res, next) => {
  try {
    const data = await prisma.storeManagementApproval.findMany({
...
// @desc    Get single management-approval by ID
// @route   GET /api/store/management-approval/:id
const getOne = async (req, res, next) => {
  try {
    const data = await prisma.storeManagementApproval.findUnique({
...
// @desc    Create management-approval
// @route   POST /api/store/management-approval
const create = async (req, res, next) => {
  try {
    const data = await prisma.storeManagementApproval.create({
...
// @desc    Upsert management-approval for its parent record (create if missing, update if it already exists)
// @route   PUT /api/store/management-approval/by-parent/:parentId
const upsertByParent = async (req, res, next) => {
  try {
    ...
    const record = await prisma.storeManagementApproval.upsert({
...
// @desc    Update management-approval
// @route   PATCH /api/store/management-approval/:id
const update = async (req, res, next) => {
  try {
    const data = await prisma.storeManagementApproval.update({
      where: { id: parseInt(req.params.id, 10) || req.params.id },
      data: sanitizeData(req.body)
    });
    res.json({ success: true, data });
...
// @desc    Delete management-approval
// @route   DELETE /api/store/management-approval/:id
const remove = async (req, res, next) => {
  try {
    await prisma.storeManagementApproval.delete({
      where: { id: parseInt(req.params.id, 10) || req.params.id }
    });
    res.json({ success: true, data: {} });
  } catch (error) {
